...
import { actChangeKeyword, actLoadAssignHubStatus, actLoadAssignOrderToShipperStatus, actLoadProducts, actLoadProvider, actLoadProviderName } from '../../../actions/index';
import { PRODUCT_ENDPOINT, PROVIDER_ENDPOINT } from '../../../api/endpoint';
...
const Toolbar = ({ onHandleFileUpload, onHandleFileChange, user, ...rest }) => {
  ...
  useEffect(() => {
    ...
    //   }
    //   API.get(`${PRODUCT_ENDPOINT}/providers/${selectedProvider}?${query}`)
    //     .then(async response => {
    //       if (response.ok) {
    //         const fetchData = await response.json();
    ...
    //     query = `page=1&limit=50&hub_id=${user.hubId}`
    //   }
    //   API.get(`${PRODUCT_ENDPOINT}?${query}`)
    //     .then(async response => {
    //       if (response.ok) {
    //         const fetchData = await response.json();
    //         const data = { invoices: fetchData.data.items, meta: fetchData.data.meta };
