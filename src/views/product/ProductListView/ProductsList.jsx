...
import { ADMIN_ENDPOINT, HUB_MANAGER_ENDPOINT, PRODUCT_ENDPOINT } from '../../../api/endpoint';
import { INVOICE_PRIORITY, ROLE } from '../../../common';
import { useSelector, useDispatch } from 'react-redux';
import { actLoadProducts } from '../../../actions/index';
...
const ProductsList = ({ data, user }) => {
  ...
  const handleChangePage = async (event, newPage) => {
    ...
    if (provider_name !== 'NONE') {
      ...
      const response = await API.get(`${PRODUCT_ENDPOINT}/providers/${provider_name}?page=${newPage + 1}&limit=50&${query}`);
      if (response.ok) {
        ...
        dispatch(actLoadProducts(dataByProvider));
        setPage(+newPage);
      }
    } else {
      let query = null;
      ...
      const response = await API.get(`${PRODUCT_ENDPOINT}?page=${newPage + 1}&limit=50&${query}`);
      if (response.ok) {
        const fetchData = await response.json();
        const data = {
          invoices: fetchData.data.items,
  ...
  const handleAssignShipper = async (shipper_id) => {
    ...
    if (response.ok) {
      ...
      if (provider_name !== 'NONE') {
        ...
        const response = await API.get(`${PRODUCT_ENDPOINT}/providers/${provider_name}?page=${page + 1}&limit=50&${query}`);
        if (response.ok) {
          const fetchData = await response.json();
        ...
        const query = `hub_id=${user.hubId}`;
        const response = await API.get(`${PRODUCT_ENDPOINT}?page=${page + 1}&limit=50&${query}`);
        if (response.ok) {
          const fetchData = await response.json();
